...
export function CourseNodeView(props: NodalViewProps & { renderCalendar: (calendar: NodeItem, embedded?: boolean) => React.ReactNode }) {
  ...
  return <section className="course-node-view">
    <div className="course-node-view__identity"><NodeTypeLabel type="curso" node={node} />
      ...
      {coverOpen && (cover ? <div id={`course-cover-${node.id}`} className="course-cover" style={coverResource ? { backgroundImage: `url(${coverResource.src})` } : undefined}><i className="corner corner--tl" /><i className="corner corner--tr" /><i className="corner corner--bl" /><i className="corner corner--br" /><NodeReference node={cover} nodes={nodes} onOpen={onOpen} /><button className="nodal-unlink" title={t("nodal.unlink")} onClick={() => onMutate((current) => withRelation(current,node.id,"cover",null))}>×</button></div> : <button className="course-cover" type="button" aria-label={t("course.cover")} onClick={() => setPicker({ role: "cover", types: ["imagen"] })}><i className="corner corner--tl" /><i className="corner corner--tr" /><i className="corner corner--bl" /><i className="corner corner--br" /><UiIcon name="image-add" /></button>)}
